...
class SRVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        super(SRVisionTransformer, self).__init__()
        ...
        self.classifier = nn.Linear(emb_dim, (patch_h//4) * (patch_w//4))
    ...
    def forward(self, x):
        ...
        x = self.dconv_up1(x)
        x = self.upsample(x)
        x_aa = self.dconv_up2(x)

        x = self.upsample(x_aa)
        x_bb = self.dconv_up3(x)
        x = self.upsample(x_bb)
        x_cc = self.dconv_up4(x)
        sur = self.avgpool(x_cc)
        # 256x256 to 16*16，and selfAttention
        x = rearrange(x_cc, 'b c (h m) (w n) -> b (c m n) (h w)', m=16, n=16)
        # embedding
        ...
        out1 = rearrange(out, 'b (c m n) (h w) -> b c (h m) (w n)', m=16, n=16, h=4, w=4)
        out = self.upsample2(out1)

        out = out + x_cc
        # out = torch.cat([out, cc], dim=1)
        out = self.dconv_down1(out)
        out = self.maxpool(out)
        out = out + x_bb
        # out = torch.cat([out, bb], dim=1)
        out = self.dconv_down2(out)
        out = self.maxpool(out)
        out = out + x_aa
        # out = torch.cat([out, aa], dim=1)
